...
R tries to transform everything into one single type. This happens most often with the elements being converted into `character` type.
...
The indexing operation does not change the type or class of the object.
...
We have to use double brakets to assign an object like a list to another list.

```{r}
y <- list(1:4, list(letters[1:5]), list(c('foo', 'type')))
y[c(3,2,1,2)]
```
...
```{r}
z <- list(keys = c('A', 'B', 'C'), values = c(10, 11, 13))
z
z[c(1,2)]
z$keys
```
...
The first line doesn't give an error in R, which may be problematic.
